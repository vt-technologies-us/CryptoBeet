...
def create_datasets(dataset, sequence_length, dim_out=1):
    seq_dataset = []
    for i in range(len(dataset) - sequence_length - dim_out):
        seq_dataset.append(dataset[i: i + sequence_length + dim_out])

    #    seq_dataset = np.array(seq_dataset)
    seq_dataset = np.array(seq_dataset, dtype=np.float16)

    data_x = seq_dataset[:, :-dim_out]
    data_y = seq_dataset[:, -dim_out:].ravel()

    return data_x, data_y


def make_model(mode='GRU', dim_out=1):
    model = Sequential()
    ...
    if mode == 'GRU':
        ...
        model.add(GRU(input_shape=(None, 1), units=100, return_sequences=True, recurrent_dropout=0.3485, use_bias=True))
        model.add(Dropout(0.0015))

        model.add(GRU(150, return_sequences=False, recurrent_dropout=0.349, use_bias=True, ))
        ...
        model.add(
            LSTM(150, return_sequences=False, recurrent_dropout=0.17, kernel_initializer='glorot_uniform',
                 use_bias=True,
...
def make_train_test(currency_close_price, look_back, dim_out=1):
    ...
    x_train, y_train = create_datasets(train, look_back, dim_out)
    x_test, y_test = create_datasets(test, look_back, dim_out)
...
def learn(db, l=0, max_size=0, mode='LSTM', model_dim_out=1):
    i = int(l / Y_NUM)
    j = l % Y_NUM
    currency = db.name
    print(l, currency)
    ...
    scaler, x_train, y_train, x_test, y_test = make_train_test(currency_close_price, look_back, model_dim_out)

    model = make_model(mode, model_dim_out)
    history = model.fit(x_train, y_train, batch_size=64, epochs=3, verbose=2, validation_split=0.25)
