...
def learn(db, l=0, max_size=0):
    ...
    history = model.fit(x_train, y_train, batch_size=64, epochs=26, verbose=2, validation_split=0.25)
...
def main():
    ...
    with tables.File(db_filename, 'r') as f:
        for l, db in enumerate(f.root):
            ...
            models[db.name].save('model/GRU_model_{}.h5'.format(db.name))  # creates a HDF5 file 'my_model.h5'
            # model = load_model('model/GRU_model_{}.h5'.format(db.name))
